...
with sqlite3.connect("./database/mock_resq.db") as conn:
    c = conn.cursor()
    c.execute(f" select COUNT(DISTINCT userid) from orders group by strftime('%Y-%m-%d',createdat) having strftime('%Y-%m-%d',createdat) in {str(tuple(public_holidays))}")

    records = c.fetchall()
    holiday_users = [record[0] for record in records]
    #for record in records:
        #print(record)

    c = conn.cursor()
    c.execute(f" select COUNT(DISTINCT userid) from orders group by strftime('%Y-%m-%d',createdat) having strftime('%Y-%m-%d',createdat) NOT in {str(tuple(public_holidays))}")

    records = c.fetchall()
    non_holiday_users = [record[0] for record in records]


res = ttest_ind(holiday_users, non_holiday_users, equal_var=False)

x1 = np.sort(holiday_users)
y1 = np.arange(len(x1))/float(len(x1))
m = np.mean(holiday_users)
s = np.std(holiday_users)
y1n = np.array([norm.cdf(x,m,s) for x in x1])
x2 = np.sort(non_holiday_users)
y2 = np.arange(len(x2))/float(len(x2))
m = np.mean(non_holiday_users)
s = np.std(non_holiday_users)
y2n = np.array([norm.cdf(x,m,s) for x in x2])
plt.plot(x1, y1, label="Holiday Purchasing Users CDF")
plt.plot(x1, y1n, label="Holiday Purchasing Users Normal Fit CDF")
plt.plot(x2,y2, label="Regular Day Purchasing Users CDF")
plt.plot(x2,y2n, label="Regular Day Purchasing Users Normal Fit CDF")
plt.title("Purchasing Users comparison between holidays and regular days")
plt.xlabel(f"Purchasing Users \n The distributions as well as the means are significantly different, the statistical test rejects equal means with p-value={res.pvalue}")
plt.legend(loc='best')
